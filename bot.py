...
@app.on_message(filters.command("imagine"), group=95)
async def generate_ai_image(_, message):
    if len(message.text.split()) < 2:
        return await message.reply_text("No Prompt Given!")
    text_to_generate = message.text.split(maxsplit=1)[1]
    reply_message = await message.reply("Generating Your Image...")

    api_url = " https://novax-ai-6765c6c528f9.herokuapp.com/"
    api_key = "Yash_Yash__@"
    ...
    try:
        response = r.post(api_url, json=payload)
        response.raise_for_status()
        api_response = response.json()

        if "image_url" in api_response:
            image_url = api_response["image_url"]
            await message.reply_photo(image_url, caption=f"**Generated BY:** @{app.me.username}")
        else:
            await message.reply_text("An error occurred while generating the image.")
